...
# Test cases
StrategyTestCase = namedtuple('Strategy',
...
strategy_test_cases = [
    ...
    StrategyTestCase(
        ...
        insample_args=dict(symbol="SINE_FAST_NOISE", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 12, 31),
                           sv=100000),
        outsample_args=dict(symbol="SINE_FAST_NOISE", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 12, 31),
                            sv=100000),
...
@pytest.mark.parametrize(
    "description, insample_args, outsample_args, benchmark_type, benchmark, impact, train_time, test_time, max_time, seed",
    strategy_test_cases)
def test_strategy(description, insample_args, outsample_args, benchmark_type, benchmark, impact, train_time, test_time,
    ...
    try:
        ...
        if len(in_trades_1.shape) != 2 or in_trades_1.shape[1] != 1:
            ...
            for date, trade in in_trades_1.iterrows():
                ...
                if (trade.iloc[0] != 0) and \
                    ...
                    incorrect = True
                    msgs.append("  holdings more than {} long or short in first insample DF. Date {}, Trade {}".format(
            ...
            for date, trade in in_trades_2.iterrows():
                ...
                if (trade.iloc[0] != 0) and \
                    ...
                    incorrect = True
                    msgs.append("  holdings more than {} long or short in second insample DF. Date {}, Trade {}".format(
                        MAX_HOLDINGS, date, trade))
                    break
            tmp_csum = 0.0
            for date, trade in out_trades.iterrows():
                ...
                if (trade.iloc[0] != 0) and \
                    ...
                    incorrect = True
                    msgs.append("  holdings more than {} long or short in out-of-sample DF. Date {}, Trade {}".format(
                        MAX_HOLDINGS, date, trade))
                    break
        ...
        if not (incorrect):
            ...
            student_insample_cr = evalPolicy2(insample_args['symbol'], in_trades_1, insample_args['sv'],
                                              insample_args['sd'], insample_args['ed'], market_impact=impact,
                                              commission_cost=0.0)
            student_outsample_cr = evalPolicy2(outsample_args['symbol'], out_trades, outsample_args['sv'],
                                               outsample_args['sd'], outsample_args['ed'], market_impact=impact,
                                               commission_cost=0.0)
            if student_insample_cr <= benchmark:
                incorrect = True
                msgs.append(
                    "  in-sample return ({}) did not beat benchmark ({})".format(student_insample_cr, benchmark))
            else:
                points_earned += 5.0
            if outsample_cr_to_beat is None:
                ...
                if student_outsample_cr < outsample_cr_to_beat:
                    incorrect = True
                    msgs.append("  out-sample return ({}) did not beat benchmark ({})".format(student_outsample_cr,
                                                                                              outsample_cr_to_beat))
                else:
                    points_earned += 5.0
        if incorrect:
            inputs_str = "    insample_args: {}\n" \
                         ...
                         "    test_time: {}\n" \
                         "    max_time: {}\n" \
                         "    seed: {}\n".format(insample_args, outsample_args, benchmark_type, benchmark, train_time,
                                                 test_time, max_time, seed)
        ...
        for i in xrange(len(tb_list)):
            row = tb_list[i]
            tb_list[i] = (
            os.path.basename(row[0]), row[1], row[2], row[3])  # show only filename instead of long absolute path
...
def evalPolicy2(symbol, student_trades, startval, sd, ed, market_impact, commission_cost):
    ...
    for row_idx in student_trades.index:
        ...
        new_row = pd.DataFrame([[abs(nshares), order, symbol], ], columns=['Shares', 'Order', 'Symbol'],
                               index=[row_idx, ])
